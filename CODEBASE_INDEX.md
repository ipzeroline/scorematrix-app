...
- `src/app/api/data/[...path]/route.ts`: same-origin data Backend BFF; proxies browser data/member requests to `DATA_BASE_URL setting` while preserving bearer and locale headers
...
- `src/lib/credits-api.ts`: typed wrapper for `GET /credits/packages`; the `/credits` page uses it to load purchasable credit packages and first-purchase bonus data from the scorm Backend.
...
- `src/lib/leagues-api.ts`: typed wrappers for `GET /leagues`, `GET /leagues/:id`, `POST /leagues`, `POST /leagues/:id/join`, `POST /leagues/:id/leave`, `POST /leagues/:id/kick/:uid`, `GET /leagues/:id/requests`, and join-request approve/reject actions; free league joins are surfaced as pending owner approval, paid joins remain immediate, leave/kick actions show the 7-day league cooldown, entry-fee aliases are normalized, full leagues cannot be joined, league lists avoid exposing backend user IDs, detail standings normalize member display name, username, avatar, level, joined date, prediction count, wins, points, and accuracy when the backend provides them, and private invite codes render only when the detail response confirms the viewer is the owner.
- `src/lib/referrals-api.ts`: typed wrapper for affiliate referral data. The `/affiliate` page prefers scorm `GET /referral` for code, clicks, signup totals, earnings, referral rows, and reward tiers, with the older `GET /referrals/code` plus `GET /referrals` flow as fallback. Invite links are normalized client-side to the current origin and locale register route `/{locale}/auth/register?ref=...`; the register page posts referral-link visits to same-origin `/api/referral-clicks`.
