...
- The homepage hero banner loads public scorm banners from `GET /banners` via `src/lib/banners-api.ts`; `src/components/home/HeroBanner.tsx` renders API-provided title, description, background image, and link URL with static translated slides only as a fallback when the API returns no banners.
