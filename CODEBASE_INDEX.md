...
- `src/lib/checkout-api.ts`: typed wrapper for credit checkout. The `/credits` page creates real backend purchases through same-origin `POST /api/data/credits/purchase`, sends `paymentMethod: "credit_card"` for the Stripe-only checkout flow plus locale-aware Stripe return URLs (`/{locale}/payment/success` and `/{locale}/payment/cancel`), and redirects to the backend-provided Stripe/payment URL. Payment return pages redirect back to `/credits` with checkout status so the wallet can refresh, using `src/lib/public-origin.ts` to prefer forwarded public host/protocol and avoid leaking internal localhost origins after reverse-proxy redirects. Credits remain server-authoritative and are applied asynchronously by the Stripe webhook (`POST /webhooks/stripe`).
