...
import { getLoginSeoContent } from "@/data/login-seo-content";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seo = getLoginSeoContent(locale);
  const pathname = `/${locale}/auth/login`;
  const canonical = `${SITE_URL}${pathname}`;
  const languages = Object.fromEntries(
    LOCALE_CODES.map((code) => [code, `${SITE_URL}/${code}/auth/login`])
  );
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...languages,
        "x-default": `${SITE_URL}/th/auth/login`,
      },
    },
    robots: {
      index: true,
...
export default async function LoginPage({ params }: Props) {
  const { locale } = await params;
  const seo = getLoginSeoContent(locale);
  const structuredData = buildLoginStructuredData(locale, seo);
...
function buildLoginStructuredData(
  locale: string,
  seo: ReturnType<typeof getLoginSeoContent>
) {
  const url = `${SITE_URL}/${locale}/auth/login`;
