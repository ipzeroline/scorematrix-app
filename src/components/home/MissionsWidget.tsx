...
export function MissionsWidget({ initialHasAuthSession = false }: MissionsWidgetProps) {
  ...
  const handleClaim = async (mission: Mission) => {
    ...
    try {
      ...
      useUserStore.setState({
        ...
        xp: nextXP,
        level: Math.min(Math.floor(nextXP / 1000) + 1, MAX_LEVEL),
  ...
  const displayLevel = Math.min(userLevel, MAX_LEVEL);
  const isMaxLevel = userLevel >= MAX_LEVEL;
  ...
  return (
    <div className="relative flex w-full flex-col gap-5">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <Card className="relative overflow-hidden border border-cyan-300/15 bg-[#0b111d]/95 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.24)] backdrop-blur-sm sm:p-5">
          ...
          <div className="relative space-y-4">
            <div className="flex items-center gap-3">
              <div className={cn(
                "flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border text-sm font-black uppercase tracking-wide",
            ...
            <div className="rounded-xl border border-white/10 bg-black/24 p-3.5">
              <div className="mb-2 flex items-center justify-between text-xs font-bold uppercase tracking-wide text-gray-400">
                ...
                <span className={cn("text-white", isMaxLevel && "text-amber-300")}>
                  {isMaxLevel ? t('profile.maxLevelReached') : `${userXP % 1000}/1000 XP`}
                </span>
              </div>
              <XPProgressBar
                currentXP={userXP}
                level={userLevel}
                maxLevel={MAX_LEVEL}
                maxLabel={t('profile.maxLevelMessage')}
                className="!space-y-0"
              />
            </div>

            <div className="grid gap-2.5">
              <div className="flex items-center justify-between rounded-xl border border-amber-300/15 bg-amber-300/[0.04] px-3 py-2.5">
                ...
                <span className="font-mono text-lg font-black leading-none text-white">{userStreak} {copy.days}</span>
              </div>
              <div className="flex items-center justify-between rounded-xl border border-emerald-300/15 bg-emerald-300/[0.04] px-3 py-2.5">
                <span className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-gray-400">
                  <CheckCircle2 size={16} className="text-emerald-400" />
                  {copy.title} {t('gamification.complete')}
                </span>
                <span className="font-mono text-lg font-black leading-none text-white">{missionsCompleted}</span>
              </div>
            </div>
          </div>
        ...
        <Card className="flex flex-col justify-between border border-cyan-300/15 bg-[#0b111d] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.24)] sm:p-5">
          <div className="space-y-5">
            <div className="grid gap-4 border-b border-white/10 pb-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
              <div className="flex min-w-0 items-center gap-3">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.12)]">
                  <Sparkles size={20} />
                </span>
                <div className="min-w-0">
              ...
              <div className="grid w-full grid-cols-3 gap-1 rounded-xl border border-white/10 bg-[#070b13] p-1 shadow-inner shadow-black/40 xl:w-[360px]">
                <button
                  onClick={() => setActiveTab('daily')}
                  className={cn(
                    'inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-transparent px-3 py-2 text-sm font-extrabold whitespace-nowrap [word-break:keep-all] transition-all duration-150',
                    activeTab === 'daily'
            ...
            {activeMissions.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
                {activeMissions.map((mission) => {
                  ...
                  const progressValue = Math.min(mission.progress, mission.target);
                  const rewardParts = [
                    mission.rewardPoints > 0 ? `+${mission.rewardPoints.toLocaleString()} PTS` : null,
                    mission.rewardXP > 0 ? `+${mission.rewardXP.toLocaleString()} XP` : null,
                  ...
                  return (
                    <div
                      key={mission.id}
                      className={cn(
                        'group flex min-h-[172px] flex-col justify-between rounded-xl border border-white/10 bg-[#080d16] p-4 transition-all duration-300',
                        styles.glow,
                    ...
                    >
                        ...
                        <div className="mb-3 flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="mb-1 flex items-center justify-between gap-3">
                              <span className={cn("truncate text-[11px] font-black uppercase tracking-wider", styles.text)}>
                                {copy.categories?.[mission.category] ?? mission.category}
                              </span>
                              {isCompleted && !isClaimed ? (
                                <span className="shrink-0 rounded-full border border-amber-400/25 bg-amber-400/10 px-2 py-0.5 text-[10px] font-black text-amber-300">
                                  {t('gamification.claim')}
                                </span>
                              ) : null}
                            </div>
                            <h4 className="line-clamp-1 text-xl font-black leading-tight text-white transition-colors duration-150 group-hover:text-cyan-200">
                              {mission.title}
                            </h4>
                            <p className="mt-1.5 line-clamp-2 text-sm font-semibold leading-6 text-gray-400">
                      ...
                      <div className="mt-4 space-y-3 border-t border-white/10 pt-3">
                        <div>
                          <div className="mb-1.5 flex items-center justify-between gap-3 text-xs font-black">
                            <span className="uppercase tracking-wide text-gray-400">
                              {isClaimed ? t('gamification.complete') : copy.progress}
                            </span>
                            <span className="font-mono text-gray-200">
                              {progressValue.toLocaleString()}/{mission.target.toLocaleString()}
                            </span>
                          </div>
                          <ProgressBar
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          {isClaimed ? (
                            <span className="flex items-center gap-1.5 rounded-lg border border-green/20 bg-green/5 px-2.5 py-1.5 text-xs font-bold text-green">
                            ...
                            <button
                              ...
                              disabled={isClaiming}
                              className={cn(
                                "flex items-center justify-center rounded-lg border px-3 py-1.5 text-xs font-black uppercase tracking-wide text-black transition-all duration-200",
                                isClaiming 
                            ...
                            <span className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-black/25 px-2.5 py-1.5 text-xs font-bold text-gray-500">
                              <Lock size={12} />
                              {copy.inProgress}
                            </span>
                          )}
